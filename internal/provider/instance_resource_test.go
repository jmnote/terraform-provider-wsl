...
//
// Run: TF_ACC=1 WSL_ACC_TEST_DISTRIBUTION=Ubuntu-24.04 go test ./internal/provider/... -v -run TestAccWSLInstanceResource_InstallMode
func TestAccWSLInstanceResource_InstallMode(t *testing.T) {
	...
	name := "testacc-wsl-instance-install-test"

	resource.Test(t, resource.TestCase{
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccInstanceInstallConfig(name, distribution),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("wsl_instance.test", "name", name),
					resource.TestCheckResourceAttr("wsl_instance.test", "distribution", distribution),
					resource.TestCheckResourceAttrSet("wsl_instance.test", "version"),
					resource.TestCheckResourceAttrSet("wsl_instance.test", "state"),
				),
			},
			{
				ResourceName:            "wsl_instance.test",
				ImportState:             true,
...
// TestAccWSLInstanceResource_ImportMode creates, updates (WSL version),
// and destroys a real WSL instance via the import creation mode
...
//     instance, only registers/reads/updates/unregisters it)
//
// Run: TF_ACC=1 WSL_ACC_TEST_ROOTFS=C:\path\to\rootfs.tar go test ./internal/provider/... -v -run TestAccWSLInstanceResource_ImportMode
func TestAccWSLInstanceResource_ImportMode(t *testing.T) {
	rootfs := os.Getenv("WSL_ACC_TEST_ROOTFS")
	if rootfs == "" {
		t.Skip("set WSL_ACC_TEST_ROOTFS to a root filesystem tar to run this acceptance test")
	}

	name := "testacc-wsl-instance-import-test"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccInstanceImportConfig(name, rootfs, location, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("wsl_instance.test", "name", name),
					resource.TestCheckResourceAttr("wsl_instance.test", "version", "2"),
					resource.TestCheckResourceAttrSet("wsl_instance.test", "state"),
				),
			},
			{
				...
				Config: testAccInstanceImportConfig(name, rootfs, location, 1),
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("wsl_instance.test", "version", "1"),
				),
			},
			{
				ResourceName:            "wsl_instance.test",
...
func testAccInstanceInstallConfig(name, distribution string) string {
	return fmt.Sprintf(`
resource "wsl_instance" "test" {
  name         = %[1]q
  distribution = %[2]q
}
`, name, distribution)
...
func testAccInstanceImportConfig(name, rootfs, location string, version int) string {
	return fmt.Sprintf(`
resource "wsl_instance" "test" {
  name     = %[1]q
  rootfs   = %[2]q
